...
class User:
    def __init__(self, name: str, params: Dict[str, int], morals: Dict[str, str], mood: float = 0.5, position: Optional[tuple] = None, memory: Optional[str] = None, relations: Optional[Dict] = None):
        self.name = name
        self.params = params
        self.morals = morals
        self.mood = mood
...
class CreateUser:
    def __init__(self, total_points: int = 50, param_names: Optional[List[str]] = None, moral_pool: Optional[List[Dict[str, str]]] = None):
        ...
        self.param_names = param_names or ['intelligence', 'strength', 'endurance', 'courage', 'perception']
        self.moral_pool = moral_pool or [{'key': 'preserve_nature', 'description': 'Береги природу и животных.'}, {'key': 'never_harm', 'description': 'Не причиняй вреда никому.'}]
    ...
    def generate_moral_principles(self) -> Dict[str, str]:
        selected_principles = random.sample(self.moral_pool, random.randint(1, 2))
        return {principle["key"]: principle["description"] for principle in selected_principles}

    def generate_character(self, name: str, legend: str) -> User:
        params = self.generate_balanced_params()
        moral_principles = self.generate_moral_principles()
        return User(name, params, moral_principles)


def create_user_character():
    name = input("Введите имя игрока: ")
    legend = input("Введите легенду персонажа: ")

    creator = CreateUser()
    user_character = creator.generate_character(name, legend)
    user_character.save_to_json()

    print(f"Персонаж {user_character.name} создан и сохранён!")
